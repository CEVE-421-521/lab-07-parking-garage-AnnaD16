...
```{julia} 
N_SOW = 10
ParkingGarageSOW(; demand_growth_rate=rand()*100, n_years=20, discount_rate=0.12)

[surge(water) for water in 3:10]
[ParkingGarageSOW(; demand_growth_rate=rand()*100, n_years=20, discount_rate=0.12) for _ in 1:N_SOW]
```
...
```{julia}
x = ParkingGarageState(5, 2024)
# policy = AdaptivePolicy(2)
# get_action(x, policy, 1.2)
# ParkingGarage.calculate_capacity(x)

# ParkingGarage.calculate_capacity( )
